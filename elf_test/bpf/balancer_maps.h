...
// map, which contains all the reals for which we are doing load balancing
struct bpf_map_def SEC("maps") local_map = {
  .type = BPF_MAP_TYPE_HASH,
...
BPF_ANNOTATE_KV_PAIR(local_map, struct address, __u32);
...
// map which contains cpu core to lru mapping
struct bpf_map_def SEC("maps") backup_lru_maps_mapping = {
  .type = BPF_MAP_TYPE_ARRAY_OF_MAPS,
  .key_size = sizeof(__u32),
  .value_size = sizeof(__u32),
  .max_entries = MAX_SUPPORTED_CPUS,
...
BPF_ANNOTATE_KV_PAIR(backup_lru_maps_mapping, __u32, __u32);
...
// map which contains opaque real's detail info
struct bpf_map_def SEC("maps") reals_info = {
  .type = BPF_MAP_TYPE_HASH,
  .key_size = sizeof(struct address),
  .value_size = sizeof(struct real_info),
  .max_entries = MAX_REALS,
  .map_flags = NO_FLAGS,
};
BPF_ANNOTATE_KV_PAIR(reals_info, struct address, struct real_info);

// map with per real and per vip pps/bps statistic
struct bpf_map_def SEC("maps") vip_rip_stats = {
  .type = BPF_MAP_TYPE_ARRAY_OF_MAPS,
  .key_size = sizeof(__u32),
  .value_size = sizeof(__u32),
  .max_entries = MAX_VIPS,
...
BPF_ANNOTATE_KV_PAIR(vip_rip_stats, __u32, __u32);

// map w/ per core statistics
struct bpf_map_def SEC("maps") core_stats = {
  .type = BPF_MAP_TYPE_PERCPU_ARRAY,
  .key_size = sizeof(__u32),
  .value_size = sizeof(union core_stats),
  .max_entries = KATRAN_CORE_MAX_STAT,
  .map_flags = NO_FLAGS,
};
BPF_ANNOTATE_KV_PAIR(core_stats, __u32, union core_stats);
...
BPF_ANNOTATE_KV_PAIR(conn_stats, __u32, struct lb_stats);
...
// map with per cpu packet info
struct bpf_map_def SEC("maps") packet_desc = {
  .type = BPF_MAP_TYPE_PERCPU_ARRAY,
  .key_size = sizeof(__u32),
  .value_size = sizeof(struct packet_description),
  .max_entries = 1,
  .map_flags = NO_FLAGS,
};
BPF_ANNOTATE_KV_PAIR(packet_desc, __u32, struct packet_description);

// map with per cpu redirect info
struct bpf_map_def SEC("maps") redirect_desc = {
  .type = BPF_MAP_TYPE_PERCPU_ARRAY,
  .key_size = sizeof(__u32),
  .value_size = sizeof(__u32),
  .max_entries = 1,
...
BPF_ANNOTATE_KV_PAIR(redirect_desc, __u32, __u32);
